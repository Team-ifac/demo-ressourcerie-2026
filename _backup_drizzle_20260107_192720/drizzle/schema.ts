import { mysqlTable, mysqlSchema, AnyMySqlColumn, int, varchar, timestamp, foreignKey, mysqlEnum, text, index } from "drizzle-orm/mysql-core"
...
export const collectionProfiles = mysqlTable("collection_profiles", {
	collectionId: int().notNull().references(() => collections.id, { onDelete: "cascade" } ),
	profileType: mysqlEnum(['animateur','formateur','directeur','stagiaire_bafa']).notNull(),
...
export const resourceThemes = mysqlTable("resource_themes", {
	resourceId: int().notNull().references(() => resources.id, { onDelete: "cascade" } ),
	themeId: int().notNull().references(() => themes.id, { onDelete: "cascade" } ),
});
...
export const resources = mysqlTable("resources", {
	...
	updatedAt: timestamp({ mode: 'string' }).defaultNow().onUpdateNow().notNull(),
	category: text(),
	status: mysqlEnum(['draft','pending','approved','rejected']).default('approved').notNull(),
	viewCount: int().default(0).notNull(),
	accessLevel: mysqlEnum(['PUBLIC','AUTHENTICATED','PREMIUM']).default('PUBLIC').notNull(),
});

export const subscriptions = mysqlTable("subscriptions", {
	id: int().autoincrement().notNull(),
	userId: int().notNull().references(() => users.id, { onDelete: "cascade" } ).references(() => users.id, { onDelete: "cascade" } ),
	stripeCustomerId: varchar({ length: 255 }).notNull(),
	stripeSubscriptionId: varchar({ length: 255 }).notNull(),
	status: mysqlEnum(['active','canceled','past_due','unpaid','incomplete']).notNull(),
...
(table) => [
	index("themes_slug_unique").on(table.slug),
]);
