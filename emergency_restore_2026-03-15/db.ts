...
export async function getAdminPlatformStats(limit: number = 10) {
  ...
  const totalDownloadsRows = await db
    .select({
      total: sql<number>`coalesce(sum(resources.downloadCount), 0)`,
    })
    .from(resources);
  ...
  const topDownloadedRows = await db
    .select({
      ...
      status: resources.status,
      accessLevel: resources.accessLevel,
      downloadCount: sql<number>`coalesce(resources.downloadCount, 0)`,
    })
    .from(resources)
    .orderBy(
      desc(sql`coalesce(resources.downloadCount, 0)`),
      desc(resources.createdAt)
    )
    .limit(safeLimit);
  ...
  if (resourceIds.length > 0) {
    ...
    const downloadCountRows = await db
      .select({
        ...
        count: sql<number>`coalesce(resources.downloadCount, 0)`,
      })
      .from(resources)
